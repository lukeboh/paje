...
- **`Ctrl+Q` não funciona numa pasta que só tem subpastas (grupo sem repositório correspondente)** — hoje exige um nó `type: "project"` com clone git real (`hasGitDir`); num grupo (pasta), mesmo que ele corresponda a um diretório local real dentro do `baseDir`, `Ctrl+Q` recusa e não faz nada. Deveria sair e mudar para o diretório local daquele grupo do mesmo jeito.
- **Não dá pra selecionar (Espaço) um item filtrado por `Ctrl+F`** — confirmado no código (`tui.app.tsx`): o handler de Espaço tem a condição `input === " " && !searchActive`, ou seja, **Espaço é ignorado sempre que o modo de pesquisa está ativo**, o que inclui olhar pra qualquer resultado já filtrado. Não há como marcar um item pra sincronizar sem sair do modo de pesquisa primeiro — e sair com `Esc` também limpa o filtro. Bloqueia o uso mais óbvio de "filtrar, depois marcar".
- **Checkout em massa (`Ctrl+K`) falha com erro confuso em repositório ainda não clonado** — confirmado no código: `checkoutOrCreateBranch`/`checkoutOrCreateBranchBulk` (`gitBranchService.ts`) rodam `git -C <targetPath> ...` sem checar antes se `targetPath` existe. Num repositório marcado que nunca foi clonado localmente, o resultado é `git: cannot change to '<path>': No such file or directory`, reportado como falha genérica em vez de um "pulado: ainda não clonado" claro (mesmo padrão que `checkoutDefaultBranchBulk` já usa para "branch padrão desconhecida").
