...
- A TUI pré-seleciona repositórios com clone local (`[x]`); grupos propagam seleção parcial (`[~]`). A verificação é feita **no disco, no momento da carga** — mesmo quando a árvore abre a partir do cache, o cache nunca decide a seleção inicial (um clone apagado manualmente entra desmarcado; um clone feito na sessão anterior entra marcado).
- Repositórios **desmarcados** nunca são clonados nem baixados pelo `Ctrl+S`. A única ação possível sobre um item desmarcado é a oferta de **remoção** da cópia local, quando ela existir.
